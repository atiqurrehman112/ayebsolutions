...
- `src/features/admin`: the administrative presentation boundary. Sprint 7A owns the static `/admin` dashboard foundation, `/admin/login` interface preview, reusable workspace shell, immutable navigation, disconnected module cards, non-numeric status widgets, and static activity states. The nested admin layout applies `noindex,nofollow` and the routes repeat that policy. No identity, authorization, session, cookie, middleware, database, API, fetch, persistence, protected route, or CMS mutation exists; future admin functionality must establish those boundaries explicitly.
