...
- `src/features/services`: the services vertical slice. Sprint 5A owns the `/services` landing presentation, while Sprint 5B owns the `/services/web-development` detail presentation, web-development content, demo-project disclosures, FAQ content, and page schema sources. Each route imports the feature through its public barrel; detail-page content remains independent from the landing-page source.
