...
- `src/lib/auth`: authentication use cases and policy. `session.ts` maps verified Supabase users and their active `profiles` row into the shared auth contract; `auth.ts` owns sign-in, sign-out, and `requireAdmin`; `permissions.ts` maps the database-owned `admin`, `editor`, and `viewer` roles into immutable capabilities. Missing or inactive profiles fail closed, and RLS evaluates the same profile authority.
...
The `/admin/contact-leads` route server-renders count-aware, paginated lead records through `ContactLeadsRepository`. Search, status/priority/assignment/date filters, deterministic sorting, assignee discovery, status history, and email history remain in the repository boundary. `src/lib/actions/contact-leads.ts` is the sole mutation and delivery boundary: it validates input with Zod, applies authenticated role checks, calls the repository, invokes the server-only Resend adapter, records successful delivery history, and revalidates the CRM route. Sprint 8H originally normalized legacy preview states into the first production pipeline; Sprint 14A subsequently evolves those values in place as documented below. Status and email events use append-only related tables so audit history is queryable without rewriting the lead record. Administrators and editors manage, assign, respond, and archive; permanent deletion remains administrator-only; viewers remain read-only; RLS remains authoritative.

## Sprint 14A Contact Leads CRM

Sprint 14A evolves the existing Contact Leads boundary without creating a second form, table, repository, or email path. Migration `202608160003_contact_leads_crm_14a.sql` renames the legacy enum values in place so existing `contacted`, `qualified`, and `proposal_sent` rows become `read`, `in_progress`, and `replied` without data loss. The same table gains important/read/replied state plus bounded request context. Only a one-way HMAC IP fingerprint is stored; raw IP addresses are never persisted.

## Sprint 14B CRM Email Center

Sprint 14B extends the existing Contact Leads boundary rather than introducing a second CRM or mail path. Migration `202608160004_crm_email_center.sql` preserves existing lead and email records while adding delivery metadata, recipient headers, direction, scheduled follow-ups, immutable note history, and reusable singleton-independent email templates. System templates are seeded idempotently; custom templates remain editable.

The protected route loads leads, filters, communication history, templates, reminders, and aggregate operational counters through `ContactLeadsRepository`. UI components never access Supabase. All email, template, note, and follow-up mutations pass through validated Server Actions, an explicit same-origin check, the existing authenticated role model, and database RLS. Communication actions are administrator-only; existing editor lead-management and viewer read-only behavior are preserved. Resend secrets remain server-only, outgoing messages are rate-limited per administrator, template variables are interpolated from the selected lead, and rich text is converted to safe generated HTML rather than accepting arbitrary markup. Incoming email records are supported in the timeline schema for a future webhook sprint, but no inbound transport is claimed here.

## Sprint 14C CRM Dashboard and Analytics

The protected `/admin/contact-leads/analytics` route is the reporting surface for the existing Contact Leads domain. A single repository call invokes the stable, security-invoker `crm_dashboard_analytics` PostgreSQL function introduced by migration `202608160005_crm_analytics.sql`. The function verifies `can_view_admin()`, respects table RLS, applies the requested reporting range once, and returns one JSON payload containing lead totals, status/source/month buckets, KPIs, follow-up windows, recent activity, smart signals, and per-user ownership metrics. This prevents N+1 access and avoids repeating large record queries in presentation components.

The route and dashboard shell remain Server Components. Only Chart.js canvases and browser-initiated CSV, Excel-compatible, and print-to-PDF exports hydrate. Viewer, editor, and administrator roles can inspect and filter analytics; viewers remain read only. The leaderboard is omitted when fewer than two eligible active users exist. Estimated revenue deliberately remains unavailable until the CRM gains an authorized numeric closed-value field; free-text budget ranges are never treated as revenue.

## Sprint 15A Premium Blog CMS

Sprint 15A retains the existing BlogRepository → Server Actions → Admin UI architecture and the existing cached public `/blog` and `/blog/[slug]` routes. Migrations `202608160006_premium_blog_cms.sql` and `202608160007_premium_blog_schema.sql` deliberately separate the PostgreSQL enum commit from its dependent schema. The Blog status lifecycle now includes scheduled publication, and articles gain canonical, OpenGraph media, comment preference, and schedule fields. `blog_article_media` provides ordered, foreign-keyed Media Library gallery relationships with public-due/editor RLS.

The editor stores a constrained plain-text authoring format inside the existing JSON content field. It supports headings, paragraphs, lists, quotes, tables, fenced code, callouts, internal/external links, buttons, and Media Library image/video references. The public Server Component parses these tokens into escaped React nodes; arbitrary HTML is neither stored as trusted markup nor injected. Heading sections generate the table of contents automatically. Slugs, reading time, canonical URLs, related articles, and adjacent navigation are derived automatically. Draft edits autosave through the existing validated Server Action, while unsaved browser navigation receives a native warning. Public repository queries expose scheduled articles only after their due timestamp, using the existing five-minute ISR and blog cache tag.

The public Contact Server Action still performs one transactional lead submission and the existing two best-effort Resend deliveries. After the stored procedure returns, the repository attaches country, referrer, user-agent, and the already-computed IP fingerprint to that lead. The admin Server Component delegates ILIKE search, status/date/service/budget/priority/assignment filters, sorting, pagination, filter-option discovery, and batched history loading to `ContactLeadsRepository`. Row and bulk mutations use authenticated Server Actions; editors manage pipeline state and replies, administrators additionally view/edit internal notes and permanently delete, and viewers remain read-only. Notes are removed from the serialized projection for non-admin roles.
...
The root layout composes the shell. Navigation structure and consent copy remain immutable in `src/config`; organization identity, contact channels, brand media, default metadata, social profiles, announcement content, footer business content, analytics identifiers, and maintenance state come from the canonical `public.site_configuration` singleton. The legacy `public.site_settings` key/value table remains migration history and has no runtime consumer.

`SiteSettingsRepository` is the only database boundary for global configuration. The five-minute `getPublicSiteSettings` cache resolves the published singleton and its fixed Media Library relationships once for Server Component consumers. Root metadata, Organization/WebSite schema, header branding, announcement, footer, contact presentation, and contact-email routing consume this projection; React presentation components never instantiate Supabase clients. The admin `/admin/site-settings` editor is restricted to administrators, validates the complete typed contract, updates the fixed UUID, and invalidates the global layout plus `site-settings` and `homepage` tags. The database retains the original fixed-ID check and adds a constant-expression unique index, so multiple configuration rows cannot be inserted.

Shell components may compose design-system primitives, but design-system primitives must not import shell components. See `docs/GLOBAL_SHELL.md` for the integration contract.

## STABILIZATION-1 release architecture

The complete migration history was replayed on a separate empty Supabase project and seeded without dashboard SQL or manual repair. The canonical runtime database boundary remains repository -> Supabase, mutations remain authenticated Server Actions -> validation -> repository -> path/tag invalidation, and public rendering remains Server Component-first with five-minute ISR where configured. `public.site_configuration` is the one runtime settings singleton; `public.site_settings` is retained solely as migration history.
